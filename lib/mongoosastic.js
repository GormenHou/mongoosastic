...
function Mongoosastic (schema, pluginOpts) {
  ...
  let resetKeyValues = options && options.resetKeyValues
  ...
  function postSave (doc) {
    ...
    if (doc) {
      ...
      if(resetKeyValues && resetKeyValues.length) {
        let keys = Object.keys(resetKeyValues);
        keys.forEach(key => {
          _doc[key] = resetKeyValues[key]
        })
      }
